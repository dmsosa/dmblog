...
import { putBackgroundColor, putEmoji, putFontColor } from "../../../service/articleService";
...
export function PopUpColors({ slug, backgroundColor, fontColor, setArticleWithNewFields } : {
    slug: string,
    ...
    return (
        ...
        <button className="btn btn-popup" name="colorPopup" onClick={() => togglePopup()}>
            color
        </button>
        <div className="colorPopup row">
            <label className="col-12">
                <input id="backgroundColor" type="color" name="backgroundColorInput" value={backgroundColor || "#51FF00"} onChange={handleColorInput}></input>
                <span>background color</span>
            </label>
            <label className="col-12">
                <input id="fontColor" type="color" name="fontColorInput" value={fontColor || "black"} onChange={handleColorInput}></input>
                <span>font color</span>
            </label>
            <button className="btn btn-primary col-12" onClick={sendColorToBackend}>Confirm</button>
        </div>
    </div>
)
}

export function PopUpEmojis({ slug, setArticleWithNewFields } : {
    slug: string,
    setArticleWithNewFields: (article: TArticle) => void
})  {
    ...
    const sendEmojiToBackend = (emoji: string) => {
        putEmoji({slug, emoji, headers})
        .then((article) => setArticleWithNewFields(article))
    }

    return (
        <div className="divPopup emojiPopup col-3" ref={ref}>
            <button className="btn btn-popup" onClick={() => {setEmojiOpen(!emojiOpen)}}>
                emoji
            </button>

            <EmojiPicker 
            className="emojiPopup" 
            style={{margin:0, padding:0, position: "absolute", boxShadow: "0 0 50px rgba(0, 0%, 0%, 0.5)", animation: "fadeIn 0.25s", backgroundColor: "#000D1C"}}
